...
impl ConfigHandler {
    ...
    pub fn new() -> ConfigHandler {
        thread_trace!("config.rs: ConfigHandler::new()");
        ConfigHandler {
    ...
    pub fn load(&mut self, config_path: &String) -> Result<(), String> {
        ...
        thread_trace!("config.rs: ConfigHandler::load(&mut self, \"{}\")", config_path);

        self.file_path = PathBuf::from(config_path);

        //Load file.
        thread_trace!("  Load file");
        let mut file = match File::open(self.file_path.clone().as_path()) {
            Ok(file) => file,
            Err(err) => {
                thread_trace!("    Return error");
        ...
        //Parse config file.
        thread_trace!("  Is file length 0 test");
        let file_length: usize = metadata.len() as usize;
        if file_length == 0 {
            thread_trace!("    true");
            thread_trace!("    New config");
            self.toml_data = toml::Table::new();
        } else {

            //Read file.
            thread_trace!("    false");
            thread_trace!("    Read file");
    ...
    pub fn validate(&mut self) {
        ...
        thread_trace!("config.rs: ConfigHandler::validate(&mut self)");

        let github_bot_name = self.get_string_required("config", "github_bot_name");
        if github_bot_name == String::new() {
            thread_crash!("Required config value of \"github_bot_name\" must be non-empty.");
        }

        let github_bot_token = self.get_string_required("config", "github_bot_token");
        if github_bot_token == String::new() {
            thread_crash!("Required config value of \"github_bot_token\" must be non-empty.");
        }

        let github_follow_repo = self.get_string_required("config", "github_follow_repo");
        if github_follow_repo == String::new() {
            thread_crash!("Required config value of \"github_follow_repo\" must be non-empty.");
        }

        let github_owner_name = self.get_string_required("config", "github_owner_name");
        if github_owner_name == String::new() {
            thread_crash!("Required config value of \"github_owner_name\" must be non-empty.");
        }

        let github_owner_token = self.get_string_required("config", "github_owner_token");
        if github_owner_token == String::new() {
            thread_crash!("Required config value of \"github_owner_token\" must be non-empty.");
        }

        let listen_port = self.get_string_required("config", "listen_port");
        if listen_port == String::new() {
            thread_crash!("Required config value of \"listen_port\" must be non-empty.");
        }

        let local_ip_address = self.get_string_required("config", "local_ip_address");
        if local_ip_address == String::new() {
            thread_crash!("Required config value of \"local_ip_address\" must be non-empty.");
        }

        let public_ip_address = self.get_string_required("config", "public_ip_address");
        if public_ip_address == String::new() {
            thread_crash!("Required config value of \"public_ip_address\" must be non-empty.");
        }

        //Ok to be empty
        let whitelist = self.get_array_required("config", "whitelist");

        thread_info!("Config validation passed.");
        thread_debug!("Config value \"github_bot_name\" =    \"{}\"", github_bot_name);
        thread_debug!("Config value \"github_bot_token\" =   \"{}\"", github_bot_token);
        thread_debug!("Config value \"github_follow_repo\" = \"{}\"", github_follow_repo);
        thread_debug!("Config value \"github_owner_name\" =  \"{}\"", github_owner_name);
        thread_debug!("Config value \"github_owner_token\" = \"{}\"", github_owner_token);
        thread_debug!("Config value \"listen_port\" =        \"{}\"", listen_port);
        thread_debug!("Config value \"local_ip_address\" =   \"{}\"", local_ip_address);
        thread_debug!("Config value \"public_ip_address\" =  \"{}\"", public_ip_address);
        thread_debug!("Config value \"whitelist\" =          {:?}",  whitelist);
    ...
    pub fn set_string(&mut self, section: &str, key: &str, val: &str) {
        ...
        thread_trace!("config.rs: ConfigHandler::set_string(&mut self, \"{}\", \"{}\", \"{}\")", section, key, val);
        thread_trace!("  Does key exist test");
        if self.toml_data.contains_key(&section.to_string()) {

            //Get data from the section, insert key-value pair, reinsert section
            thread_trace!("    true");
            thread_trace!("    Get data, insert key-vaule pair, reinsert");
            ...
            thread_trace!("    false");
            thread_trace!("    Create section and insert");
    ...
    pub fn set_array(&mut self, section: &str, key: &str, val: &toml::Array) {
        ...
        thread_trace!("config.rs: ConfigHandler::set_array(&mut self, \"{}\", \"{}\", \"{:?}\")", section, key, val);
        thread_trace!("  Does key exist test");
        if self.toml_data.contains_key(&section.to_string()) {

            //Get data from the section, insert key-value pair, reinsert section
            thread_trace!("    true");
            thread_trace!("    Get data");
            let mut section_data = self.toml_data.get(&section.to_string()).unwrap().as_table().unwrap().clone();
    ...
    pub fn get_string(&mut self, section: &str, key: &str) -> Result<String, String> {
        ...
        thread_trace!("config.rs: ConfigHandler::get_string(&mut self, \"{}\", \"{}\")", section, key);
        thread_trace!("  Does section exist test");
        if self.toml_data.contains_key(&section.to_string()) {
            ...
            thread_trace!("    Does key-value pair exists test");
            if section_data.contains_key(&key.to_string()) {

                thread_trace!("      true");

                //Is it a string? If not return err.
                thread_trace!("      Get data as string");
    ...
    pub fn get_array(&mut self, section: &str, key: &str) -> Result<Vec<toml::Value>, String> {
        ...
        thread_trace!("config.rs: ConfigHandler::get_array(&mut self, \"{}\", \"{}\")", section, key);
        ...
        if self.toml_data.contains_key(&section.to_string()) {
            ...
            //Does the key/value pair exist? If not create it and insert empty string for given key.
            thread_trace!("    Does key-value pair exist test");
            if section_data.contains_key(&key.to_string()) {

                thread_trace!("      true");

                //Is it an array? If not return err.
                thread_trace!("      Get data as array");
